...
def main():
    ...
    test_dataloader_default_args = dict(
        samples_per_gpu=args.num_per_gpu, workers_per_gpu=args.num_per_gpu, dist=distributed, shuffle=False)
    ...
    if cfg.get('replace_relu_with_gelu', False):
    ...
    test_adv_cfg = cfg.get("test_adv_cfg")
    if args.no_adv:
        test_adv_cfg["adv_flag"] = False
    #test_adv_cfg["num_steps"] = args.num_steps
    #test_adv_cfg["adv_type"] = args.adv_type

    if not distributed:
        model = build_dp(model, cfg.device, device_ids=cfg.gpu_ids)
        outputs = single_gpu_test(model, data_loader, args.show, args.show_dir,
                                  args.show_score_thr, test_adv_cfg=test_adv_cfg)
    else:
        model = build_ddp(
            model,
            cfg.device,
        ...
        outputs = multi_gpu_test(
            ...
            or cfg.evaluation.get('gpu_collect', False), test_adv_cfg=test_adv_cfg)
